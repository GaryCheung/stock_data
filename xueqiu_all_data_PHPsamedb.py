...
def get_stock_amplitude(stock_list,source):
    ...
    for i in range(1,len(stock_list)+1):
        ...
        for quantity,amplitude,name,pclose,phigh,plow,popen in zip(stock_quantity,stock_amplitude,stock_name,price_close, price_high, price_low, price_open):
            ...
            connection = pymysql.connect(**config_win)
            try:
                with connection.cursor() as cursor:
                    # 执行sql语句，插入记录
                    sql = 'INSERT INTO stock_data (date, quantity, amplitude, stock_name, source, price_close, price_high, price_low, price_open, weekday) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)'
                    cursor.execute(sql, (present_date, quantities, amplitudes, names, source, pclose, phigh, plow, popen, weekday))
                    # 没有设置默认自动提交，需要主动提交，以保存所执行的语句
                connection.commit()
            finally:
                connection.close()
...
stock_list_tmp=stock_list[point:]
>>>>>>> Stashed changes
